...
my.cluster <- parallel::makeCluster(5, type = "PSOCK", outfile = "./saved_results/log.txt")
...
reslist <- foreach(i=1:9) %dopar% {
  ...
  tmp_plot <- data %>% mutate("smooth"=eval.fd(data$time,smooth_current$fd),
                              "diff" = c(0,diff(data$current)),
                              "grad"=grad,
                              id=1:length(data$time)) %>% 
    filter(id%%100==0) %>% 
    pivot_longer(c(current,smooth,diff,grad),names_to = "type",values_to = "value") %>%
