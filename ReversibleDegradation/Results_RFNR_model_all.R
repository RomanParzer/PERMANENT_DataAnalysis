...
# res <- readRDS("../saved_results/results_RFNR_model_HAC_weave.rds")
# res <- readRDS("../saved_results/results_RFNR_model_autostart_val.rds")
# final version: 
res <- readRDS("./saved_results/results_RFNR_model_fixed_start.rds")
...
# # table of parameter estimates and CIs, not used
...
for (i in 1:6) {
  df_parCIs <- rbind(df_parCIs,
                     data.frame(data=c(datanames[name_order],datanames[name_order]),material=c(rep("MEA1",13),rep("MEA2",13)),
...
# log_c1    -1.5158e+02  3.1930e+03  -0.0475   0.9621
df_parCIs[8+13,c(6)] <- Inf 
# upper
# PM filter sample 2 MEA2 3.262722
...
# RH100 MEA21 -0.0002135954 -0.002896514 0.002469323
...
# estimate     lower    upper
# RH100 MEA22 150.0031 -3043.197 3343.203
df_parCIs[8+13+26*2,c(6)] <- Inf
# upper
# PM filter sample 2 MEA22 13.17408

df_parCIs[13+26*3,c(5,6)] <- c(-Inf,Inf)
# lower       upper
# RH153 -0.0006437721 0.001349724
...
for (i in 1:nrow(res$model_fits)) {
  tmp_name <- c(res$datanames[name_order[1:13]],res$datanames[name_order[1:13]])[i]
...
deg_curves %>% filter(time>1e3, !(material=="MEA1" & data=="RH15")) %>%
  ...
  theme(strip.text.x = element_blank()) +
  geom_text(data=deg_curves %>% filter(time>1e5,data=="ALL Filters sample 2",time>106001, time < 106002),
...
# # 
# # # PM filter MEA1 upper bound too high, set to Inf
# # df_degCIs[c(34),c(6)] <- Inf
# 
# df_degCIs %>% 
#   ggplot(aes(x=estimate,y=data,col=material)) +
#   geom_point() +
