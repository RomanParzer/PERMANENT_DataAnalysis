...
for (i in 1:numdat) {
  ...
  # tmp_plot
  ggsave(paste0("../plots_rev_deg/JumpsCloseMEA2_",datanames[i],".pdf"),tmp_plot,width = 8,height = 5)
  ...
  tmp_plot <- data %>% mutate(id=1:length(data$time)) %>%
    ...
    geom_vline(xintercept=data$time[ind],linetype=2,linewidth=0.1)
  # tmp_plot
  ggsave(paste0("../plots_rev_deg/JumpsMEA2_",datanames[i],".pdf"),tmp_plot,width = 8,height = 5)
  ...
  tmp_plot <- plot(model_baseline) +
    labs(x="time in s")
  # tmp_plot
  ggsave(paste0("../plots_rev_deg/Fitted_RFNR_MEA2_",datanames[i],".pdf"),tmp_plot,width = 8,height = 5)

  tmp_plot <- plot(model_baseline,"residuals") +
    labs(x="time in s")
  # tmp_plot
  ggsave(paste0("../plots_rev_deg/Residuals_RFNR_MEA2",datanames[i],".pdf"),tmp_plot,width = 8,height = 5)

  res_tab[data_order[i],1] <- length(which(ind))
  res_tab[data_order[i],2] <- mean(diff(data$time[which(ind)]))
  res_tab[data_order[i],3]<- c(rep(30,8),100,50,30,15)[i]
...
saveRDS(list(res_tab=res_tab,
        ...
        "../saved_results/results_RFNR_model_MEA2.rds")
