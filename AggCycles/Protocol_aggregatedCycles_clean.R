...
foreach(idat=1:numdat) %dopar% {
  # read in data
  if (idat <= 7) {
    data <- read_xlsx("./data/MEA_1_Currents_AST.xlsx",sheet= idat)
  } else {
    data <- read_xlsx("./data/MEA_2_Currents_AST.xlsx",sheet= idat-7)
  }
  ...
  for (i in 1:num_clean) {
    ...
    # k <- 1
    for (k in 1:M) {
      uniroot_res <- try(uniroot(function(tmp_t) {(-exp(coefk[k,2])*tmp_t - exp(coefk[k,3])*(1-exp(-exp(coefk[k,4])*tmp_t)))-log(0.7)},
      ...
      sim_values_rd_t50[k] <- exp(-exp(coefk[k,2])*50 - exp(coefk[k,3])*(1-exp(-exp(coefk[k,4])*50)))
    }
    uniroot_res <- try(uniroot(function(tmp_t) {(-exp(coefk[k,2])*tmp_t - exp(coefk[k,3])*(1-exp(-exp(coefk[k,4])*tmp_t)))-log(0.7)},
                               interval = c(0,1e3),extendInt = "downX",tol = 1e-2))
    if (inherits(uniroot_res,what = "try-error")) {
    ...
    rd_t50[i] <- exp(-exp(coefk[k,2])*50 - exp(coefk[k,3])*(1-exp(-exp(coefk[k,4])*50)))
