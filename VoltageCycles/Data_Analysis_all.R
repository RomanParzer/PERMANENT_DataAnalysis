...
tmp_t <- 0
for (i in 1:length(data$Time2)) {
  if (log_vec[i]) { # change
    tmp_t <- time_last_volt_change[i] <- c(data$Time2)[i]
  } else { # no change
    time_last_volt_change[i] <- tmp_t
  }
  if (i%%1000==0) {
    sprintf("Finished rep %d / %d!",i,length(data$Time2))
  }
}

# df_long <- df_long %>% mutate(time_v_ch=time - time_last_voltage_change)
...
tmp_gg
# ggsave("./plots/power_one_cycle.pdf",tmp_gg,height = 4,width = 7)
...
model_cycles <- lm(log(current)~time_c+I(voltage<cutoff)+I(-exp(time_c/5e3)),data=tmp_data)
model_cycles
...
require(knitr)
kable(format(as.matrix(restab[1:7,]),digits=3),format = "latex",booktabs=TRUE)
...
df_cycles_long <- df_cycles %>% pivot_longer(c(avg_hi,avg_lo,max),names_to = "variable",values_to = "value")
  mv_model <-  lmer(log(value) ~ 0 + variable + variable:(time + time_clean + I(time_clean^2)) + (1|cleaning),
                  data=df_cycles_long)
  # mv_model <-  lmer(log(value) ~ 0 + variable + variable:(time + time_clean) + (1|cleaning),
  #                   data=df_cycles_long)
# 
...
tmpy <- RE$cleaning$`(Intercept)`
plot(1:11,tmpy)
abline(lm(tmpy~c(1:11)),col=4)
